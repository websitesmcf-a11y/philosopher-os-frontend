...
import { Webhook, ArrowLeft, Plus } from 'lucide-react';
  ...
  return (
    <div className="page-content fade-in">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, letterSpacing: '-0.03em', margin: 0 }}>Webhooks</h1>
          <p style={{ fontSize: 14, color: 'var(--foreground-secondary)', marginTop: 4 }}>
            Configure outgoing webhooks
          </p>
        </div>
        <button className="btn btn-primary"><Plus size={16} /> Add Webhook</button>
      </div>
      <div className="card" style={{ padding: 40, textAlign: 'center' }}>
        <Webhook size={32} color="var(--muted)" style={{ marginBottom: 12, opacity: 0.5 }} />
        <p style={{ color: 'var(--muted)' }}>Webhook configuration coming soon</p>
      </div>
    </div>
  );
}
