...
export function CircularLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return <div className={cn("border-primary animate-spin rounded-full border-2 border-t-transparent", sizeClasses[size], className)}><span className="sr-only">Loading</span></div>
}
  ...
  return (
    <div className={cn("relative", sizeClasses[size], className)}>
      <div className="absolute h-full w-full">
        {[...Array(12)].map((_, i) => (
          <div key={i} className="bg-primary absolute animate-[spinner-fade_1.2s_linear_infinite] rounded-full"
            style={{ top: "0", left: "50%", marginLeft: size === "sm" ? "-0.75px" : size === "lg" ? "-1.25px" : "-1px",
              transformOrigin: `${size === "sm" ? "0.75px" : size === "lg" ? "1.25px" : "1px"} ${size === "sm" ? "10px" : size === "lg" ? "14px" : "12px"}`,
              transform: `rotate(${i * 30}deg)`, opacity: 0, animationDelay: `${i * 0.1}s`,
              height: barSizes[size].height, width: barSizes[size].width }} />
        ))}
      </div>
...
export function PulseLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  const sizeClasses = { sm: "size-4", md: "size-5", lg: "size-6" }
  return <div className={cn("relative", sizeClasses[size], className)}><div className="border-primary absolute inset-0 animate-[thin-pulse_1.5s_ease-in-out_infinite] rounded-full border-2" /><span className="sr-only">Loading</span></div>
}
...
export function DotsLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  return (
    <div className={cn("flex items-center space-x-1", { "h-4": size==="sm", "h-5": size==="md", "h-6": size==="lg" }, className)}>
      {[...Array(3)].map((_, i) => (
        <div key={i} className={cn("bg-primary animate-[bounce-dots_1.4s_ease-in-out_infinite] rounded-full", { "h-1.5 w-1.5": size==="sm", "h-2 w-2": size==="md", "h-2.5 w-2.5": size==="lg" })}
...
export function TypingLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  return (
    <div className={cn("flex items-center space-x-1", { "h-4": size==="sm", "h-5": size==="md", "h-6": size==="lg" }, className)}>
      {[...Array(3)].map((_, i) => (
        <div key={i} className={cn("bg-primary animate-[typing_1s_infinite] rounded-full", { "h-1 w-1": size==="sm", "h-1.5 w-1.5": size==="md", "h-2 w-2": size==="lg" })}
...
export function WaveLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  ...
  return (
    <div className={cn("flex items-center gap-0.5", { "h-4": size==="sm", "h-5": size==="md", "h-6": size==="lg" }, className)}>
      {[...Array(5)].map((_, i) => (
        <div key={i} className={cn("bg-primary animate-[wave_1s_ease-in-out_infinite] rounded-full", { "w-0.5": size==="sm"||size==="md", "w-1": size==="lg" })}
...
export function BarsLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  return (
    <div className={cn("flex", { "h-4 gap-1": size==="sm", "h-5 gap-1.5": size==="md", "h-6 gap-2": size==="lg" }, className)}>
      {[...Array(3)].map((_, i) => (
        <div key={i} className={cn("bg-primary h-full animate-[wave-bars_1.2s_ease-in-out_infinite]", { "w-1": size==="sm", "w-1.5": size==="md", "w-2": size==="lg" })}
...
export function TerminalLoader({ className, size = "md" }: { className?: string; size?: "sm" | "md" | "lg" }) {
  return (
    <div className={cn("flex items-center space-x-1", { "h-4": size==="sm", "h-5": size==="md", "h-6": size==="lg" }, className)}>
      <span className={cn("text-primary font-mono", { "text-xs": size==="sm", "text-sm": size==="md", "text-base": size==="lg" })}>{">"}</span>
      <div className={cn("bg-primary animate-[blink_1s_step-end_infinite]", { "h-3 w-1.5": size==="sm", "h-4 w-2": size==="md", "h-5 w-2.5": size==="lg" })} />
      <span className="sr-only">Loading</span>
    </div>
  )
}
...
export function TextBlinkLoader({ text = "Thinking", className, size = "md" }: { text?: string; className?: string; size?: "sm" | "md" | "lg" }) {
  return <div className={cn("animate-[text-blink_2s_ease-in-out_infinite] font-medium", { "text-xs": size==="sm", "text-sm": size==="md", "text-base": size==="lg" }, className)}>{text}</div>
}

export function TextShimmerLoader({ text = "Thinking", className, size = "md" }: { text?: string; className?: string; size?: "sm" | "md" | "lg" }) {
  return (
    <div className={cn("bg-[linear-gradient(to_right,var(--muted-foreground)_40%,var(--foreground)_60%,var(--muted-foreground)_80%)] bg-size-[200%_auto] bg-clip-text font-medium text-transparent animate-[shimmer_4s_infinite_linear]", { "text-xs": size==="sm", "text-sm": size==="md", "text-base": size==="lg" }, className)}>
      {text}
    </div>
  )
}
...
export function TextDotsLoader({ className, text = "Thinking", size = "md" }: { className?: string; text?: string; size?: "sm" | "md" | "lg" }) {
  return (
    <div className={cn("inline-flex items-center", className)}>
      <span className={cn("text-primary font-medium", { "text-xs": size==="sm", "text-sm": size==="md", "text-base": size==="lg" })}>{text}</span>
